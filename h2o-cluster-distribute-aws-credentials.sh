...
i=0
echo ${AWS_SSH_PRIVATE_KEY_FILE}
echo ${coreSiteFileName}

for publicDnsName in $(cat nodes-public)
do
    i=$((i+1))
    echo "Copying aws credential files to node ${i}: ${publicDnsName}"
#Must mkdir .ec2 first
    ssh -o StrictHostKeyChecking=no -i ${AWS_SSH_PRIVATE_KEY_FILE} ec2-user@${publicDnsName} "mkdir .ec2"
    scp -p -o StrictHostKeyChecking=no -i ${AWS_SSH_PRIVATE_KEY_FILE} "start-h2o-bg.sh" ec2-user@${publicDnsName}:
